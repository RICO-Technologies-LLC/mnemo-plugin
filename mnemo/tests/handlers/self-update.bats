...
# legacy installs (~/.claude/mnemo/ without a .claude-plugin/ subdirectory).
#
...
setup() {
    TEST_PLUGIN_DIR="$TEST_TMPDIR/test-plugin/mnemo"
...
EOF
    ...
    # Isolate HOME so we do not touch the developer's real ~/.claude/mnemo/
    export HOME="$TEST_TMPDIR/fakehome"
    mkdir -p "$HOME"

    # Stamp the debounce marker file in the future so it does not pre-exist.
    rm -f "$TMPDIR/.mnemo-update-checked"
}
...
@test "self-update: stderr messages replace the old silent-exit behavior" {
    ...
    run bash "$TEST_PLUGIN_DIR/hooks-handlers/self-update.sh"
    [[ "$status" -eq 0 ]]
    # Output must include the "mnemo self-update:" prefix from the log() function.
    [[ "$output" == *"mnemo self-update:"* ]]
}
...
EOF
    ...
    touch "$TMPDIR/.mnemo-update-checked"
