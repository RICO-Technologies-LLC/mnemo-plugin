#!/usr/bin/env bats
# json-parsing.bats — Test _mmry_parse_json_value.
...
@test "parse_json_value: extracts string value from simple JSON" {
    ...
    local result
    result="$(_mmry_parse_json_value "$json" "name")"
    [[ "$result" == "Alice" ]]
}
...
@test "parse_json_value: returns empty for missing key" {
    ...
    local result
    result="$(_mmry_parse_json_value "$json" "email")"
    [[ "$result" == "" ]]
}
...
@test "parse_json_value: extracts URL value" {
    ...
    local result
    result="$(_mmry_parse_json_value "$json" "apiUrl")"
