...
# ── _mmry_json_escape with unicode ──
...
@test "json_escape: preserves accented characters" {
    local result
    result="$(_mmry_json_escape 'café résumé')"
    [[ "$result" == 'café résumé' ]]
}

@test "json_escape: preserves CJK characters" {
    local result
    result="$(_mmry_json_escape '日本語テスト')"
    [[ "$result" == '日本語テスト' ]]
}

@test "json_escape: preserves emoji" {
    local result
    result="$(_mmry_json_escape 'test 🧠 memory')"
    [[ "$result" == 'test 🧠 memory' ]]
}
...
@test "json_escape: uses printf not echo (no -n mangling)" {
    ...
    local result
    result="$(_mmry_json_escape '-n test')"
    [[ "$result" == '-n test' ]]
}

# ── _mmry_build_json with unicode values ──

@test "build_json: handles unicode values" {
    local result
    result="$(_mmry_build_json "topic" "café" "content" "résumé")"
    [[ "$result" == '{"topic":"café","content":"résumé"}' ]]
}

@test "build_json: handles mixed unicode and escapes" {
    local result
    result="$(_mmry_build_json "content" 'He said "café"')"
